...
const postOrgDetails = async (req, res) => {
  ...
  const org = await db
    .select()
    .from(organizations)
    .where(eq(organizations.org_slug, newOrg.org_slug));
  if (org && org.length > 0) {
    return res
      .status(400)
      .send({ message: "Organization with this slug already exists" });
  }
...
const updateOrgDetails = async (req, res) => {
  try {
    ...
    console.log("Updating org:", orgDetails.org_id, updatedOrg);
    await db
      .update(organizations)
      .set(updatedOrg)
      .where(eq(organizations.org_id, orgDetails.org_id));
